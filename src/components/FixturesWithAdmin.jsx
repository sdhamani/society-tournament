...
export default function FixturesWithAdmin() {
  ...
  const getFilteredMatches = () => {
    if (selectedDay === 'saturday') {
      return matches.filter(m => m.match_number?.startsWith('SAT'))
    } else {
      return matches.filter(m => m.match_number?.startsWith('SUN'))
    }
  }
            ...
            <tbody>
              {filteredMatches.map((fixture) => (
                <tr key={fixture.id} className={fixture.group_name === 'Ceremony' ? 'ceremony' : ''}>
                  <td className="match-num">{fixture.match_number}</td>
